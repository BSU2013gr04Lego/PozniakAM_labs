    ...
    QWidget(parent)
{
    ...
    {
        if (i == SCREENSAVER)
            type->addItem(QString("ScreenSaver"));
        else if (i == SS_TRIANGLE)
    ...
    QLabel *lbl_type;
    if (wdgt_type == SCREENSAVER)
        lbl_type = new QLabel(QString("ScreenSaver"));
    else if (wdgt_type == SS_TRIANGLE)
...
void SS_container::slotUpdateWdgtGrid()
{
    if (wdgt_type == SCREENSAVER)
    {
        ...
        ssavers[selected] = new ScreenSaver(this);
